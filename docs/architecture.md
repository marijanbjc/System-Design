...
|---|---|---|
| Обученный энкодер + голова классификатора | голова поверх локальных эмбеддингов, синтетический датасет | контрастивно обученный энкодер + голова (`ml.md`) |
| Внешняя LLM API | детерминированный мок по схеме `LLMDraft`, реальный клиент по env-переменной | реальный провайдер + rate limiter и предохранитель |
